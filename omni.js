...
const unusedCampaignBags = {
  ...
  dunwich: {
    easy: ["+1", "+1", "0", "0", "0", "-1", "-1", "-1", "-2", "-2", "skull", "skull", "elderthing", "cultist", "autofail", "eldersign"],
    standard: ["+1", "0", "0", "-1", "-1", "-1", "-2", "-2", "-3", "-4", "skull", "skull", "elderthing", "cultist", "autofail", "eldersign"],
    hard: ["0", "0", "0", "-1", "-1", "-2", "-2", "-3", "-3", "-4", "-5", "skull", "skull", "skull", "elderthing", "cultist", "autofail", "eldersign"],
    expert: ["0", "-1", "-1", "-2", "-2", "-3", "-3", "-4", "-4", "-5", "-6", "-8", "skull", "skull", "skull", "elderthing", "cultist", "autofail", "eldersign"]
  },
  carcosa: {
...
const specialTokenDescriptors = {
  ...
  default: {
    skull: "No additional effect is configured for this token in the selected scenario.",
    cultist: "No additional effect is configured for this token in the selected scenario.",
    tablet: "No additional effect is configured for this token in the selected scenario.",
    elderthing: "No additional effect is configured for this token in the selected scenario.",
    eye: "No additional effect is configured for this token in the selected scenario.",
    eldersign: "Resolve this investigator's Elder Sign effect.",
    autofail: "This test automatically fails. Do not calculate a final skill value."
  }
};

// Scenario reference cards, transcribed for the three scenarios exposed by each campaign selector.
// Easy/Standard and Hard/Expert use the two sides of each official reference card.
const scenarioTokenDescriptors = {
  notz: {
    scen1: { easy: { skull: '-X, where X is Ghouls at your location.', cultist: '-1; if you fail, take 1 horror.', tablet: '-2; if a Ghoul is at your location, take 1 damage.' }, hard: { skull: '-2; if you fail, draw a Ghoul enemy from the encounter deck/discard.', cultist: 'Reveal another token; if you fail, take 2 horror.', tablet: '-4; if a Ghoul is at your location, take 1 damage and 1 horror.' } },
    scen2: { easy: { skull: '-X, where X is the highest doom on a Cultist enemy.', cultist: '-2; place 1 doom on the nearest Cultist enemy.', tablet: '-3; if you fail, place 1 of your clues on your location.' }, hard: { skull: '-X, where X is total doom in play.', cultist: '-2; place 1 doom on each Cultist. If none are in play, reveal another token.', tablet: '-4; if you fail, place all your clues on your location.' } },
    scen3: { easy: { skull: '-X, where X is Monster enemies in play.', cultist: '-2; place 1 doom on the nearest enemy.', tablet: '-3; if a Monster is at your location, take 1 damage.', elderthing: '-5; if an Ancient One is in play, reveal another token.' }, hard: { skull: '-3; if you fail, draw a Monster enemy from the encounter deck/discard.', cultist: '-4; place 2 doom on the nearest enemy.', tablet: '-5; if a Monster is at your location, take 1 damage and 1 horror.', elderthing: '-7; if an Ancient One is in play, reveal another token.' } }
  },
  dunwich: {
    scen1: { easy: { skull: '-1; if you fail, discard the top 3 cards of your deck.', cultist: '-1 (-3 instead with 10+ cards in your discard pile).', elderthing: '-X; discard the top 2 cards of your deck. X is their total printed cost.' }, hard: { skull: '-2; if you fail, discard the top 5 cards of your deck.', cultist: '-1 (-5 instead with 10+ cards in your discard pile).', elderthing: '-X; discard the top 3 cards of your deck. X is their total printed cost.' } },
    scen2: { easy: { skull: '-2; you may spend 2 resources to treat it as 0.', cultist: '-3; if you succeed, gain 3 resources.', tablet: '-2; if you fail, discard 3 resources.' }, hard: { skull: '-3; you may spend 3 resources to treat it as 0.', cultist: '-3; if you fail, discard 3 resources.', tablet: '-2; discard 3 resources.' } },
    scen3: { easy: { skull: '-1 (-3 instead if Hunting Horror is at your location).', cultist: '-1; if you fail, spawn Hunting Horror at your location if able.', tablet: '-2; return 1 clue to your location.', elderthing: '-3; if you fail, discard an asset you control.' }, hard: { skull: '-2 (-4 instead if Hunting Horror is at your location).', cultist: '-3; if you fail, spawn Hunting Horror at your location if able.', tablet: '-4; Hunting Horror at your location immediately attacks you.', elderthing: '-5; if you fail, discard an asset you control.' } }
  },
  carcosa: {
    scen1: { easy: { skull: '-1 (-3 instead with 3+ horror on you).', elderthing: '-4; add horror to your location, or take horror if it already has horror.' }, hard: { skull: '-X, where X is horror on you (minimum 1).', elderthing: '-5; add horror to your location, or take horror if it already has horror.' } },
    scen2: { easy: { skull: 'Reveal another token; if you fail, place doom on a Possessed enemy.', cultist: '-2; if you fail, place 1 clue on your location.', tablet: '-4; if you fail, take 1 horror.', elderthing: '-X, where X is your location\'s shroud.' }, hard: { skull: 'Reveal another token; if you fail, place doom on the Possessed enemy with most health.', cultist: '-3; place 1 clue on your location.', tablet: '-4; take 1 horror.', elderthing: '-X, where X is your location\'s shroud; if you fail, take 1 damage.' } },
    scen3: { easy: { skull: '-X, where X is the highest doom on an enemy.', cultist: '-2; if you fail, place doom on the nearest enemy.', tablet: '-2; if you fail, discard a random card from hand.', elderthing: '-2; if you fail with an enemy at your location, take 1 horror.' }, hard: { skull: '-X, where X is total doom on enemies.', cultist: '-4; place doom on the nearest enemy.', tablet: '-4; discard a random card from hand.', elderthing: '-4; if an enemy is at your location, take 1 horror.' } }
  }
};

const scenarioLabels = {
  notz: ['The Gathering', 'The Midnight Masks', 'The Devourer Below'],
  dunwich: ['Extracurricular Activity', 'The House Always Wins', 'The Miskatonic Museum'],
  carcosa: ['Curtain Call', 'The Last King', 'Echoes of the Past'],
  ash: ['Brethren of Ash / Intro', 'Chapter II', 'Chapter III']
};

function syncCampaignDetails() {
  const campaign = document.getElementById('campaign-select').value;
  const labels = scenarioLabels[campaign] || scenarioLabels.ash;
  Array.from(document.getElementById('scenario-select').options).forEach((option, index) => {
    option.textContent = labels[index] || option.textContent;
  });
  syncDifficultyBagLayout();
}

function getScenarioTokenEffect(campaign, scenario, difficulty, token) {
  const difficultyBand = ['hard', 'expert'].includes(difficulty) ? 'hard' : 'easy';
  return scenarioTokenDescriptors[campaign]?.[scenario]?.[difficultyBand]?.[token]
    || specialTokenDescriptors.default[token];
}
...
function drawSimpleToken(forcedToken) {
  ...
  const activeScenario = document.getElementById('scenario-select').value;
  const activeDifficulty = document.getElementById('difficulty-select').value;
  const tokenEffect = getScenarioTokenEffect(activeCampaign, activeScenario, activeDifficulty, drawnToken);
...
function updateBonusSkillVisual(skill) {
  ...
  bonusRow.style.setProperty('--bonus-skill-color', visual.color);
  bonusRow.style.setProperty('--bonus-skill-tint', visual.background);
